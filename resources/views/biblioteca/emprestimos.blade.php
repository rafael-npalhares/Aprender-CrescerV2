{{-- resources/views/biblioteca/emprestimos.blade.php --}}
{{-- Controller: BibliotecaController@meusEmprestimos --}}
@extends('layouts.admin')
@section('titulo', 'Meus Empréstimos')
...
<div class="emp-header">
    ...
    <p>Acompanhe seus livros emprestados. A devolução é confirmada pelo administrador ao receber o livro.</p>
</div>
                    ...
                    <td>
                        @if($emp->status !== 'devolvido')
                            <div class="emp-actions">
                                <form action="{{ route('biblioteca.renovar', $emp) }}" method="POST">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="btn-acao" {{ $emp->renovacoes >= 5 ? 'disabled' : '' }}>
                                        Renovar ({{ $emp->renovacoes }}/5)
                                    </button>
                                </form>
                            </div>
                        @endif
